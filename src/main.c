...
static bool load_core(LibCoreAPI* api, lstring src_dll) {
    ...
    api->current_path = copied;
    swiss_log_info("LIBCORE: %s: loaded successfully", api->current_path);
...
static bool unload_core(LibCoreAPI* api) {
    ...
    if (remove(api->current_path)) {
        error_code = FAILED_TO_DELETE_FILE;
        status = false;
        swiss_log_error("%s", api->current_path);
    }
    api->current_path = NULL;

    if (status) swiss_log_info("BOOTSTRAP: libcore unloaded successfully");
    return status;
}
